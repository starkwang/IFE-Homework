...
var Category = React.createClass({
	render : function(){
		var catagoryNodes = this.props.data.map(function(category){
			...
			return (
				<div className="categoryItem" id={_category.id} onClick={GotoTask} onTouchEnd={GotoTask}>
					{category.title}
				</div>
			);
		});
...
var Task = React.createClass({
	render : function(){
		var taskNodes = this.props.model.map(function(task){
			...
			return(
				<div className="taskItem" id={_task.id} onClick={GotoContent} onTouchEnd={GotoContent}>
					{task.title}
				</div>
			);
		})
